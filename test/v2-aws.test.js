...
describe('composeTaskDefinition DD_VERSION injection', () => {
  const image = `${REGISTRY}/hoax@sha256:new`
  const secrets = [{ name: 'DATABASE_URL', valueFrom: '/ecs/hoax/prod/DATABASE_URL' }]

  // App container's `environment` (plain [{name,value}] vars) is optional in the
  // template; the sidecar carries its own DD_VERSION to prove it is never touched.
  function base (appEnvironment) {
    return {
      family: 'hoax-prod-web',
      containerDefinitions: [
        { name: 'app', image: 'scratch', secrets: [], ...(appEnvironment ? { environment: appEnvironment } : {}) },
        {
          name: 'fluentbit',
          image: 'amazon/aws-for-fluent-bit:latest',
          environment: [{ name: 'DD_VERSION', value: 'sidecar-untouched' }]
        }
      ]
    }
  }

  it('injects DD_VERSION on the app container, creating the environment array when absent', () => {
    const composed = composeTaskDefinition(base(), { projectName: 'hoax', image, secrets, version: 'release-2026-07-23-10057' })
    expect(composed.containerDefinitions[0].environment).toEqual([{ name: 'DD_VERSION', value: 'release-2026-07-23-10057' }])
  })

  it('replaces an existing DD_VERSION entry without duplicating, preserving other env vars', () => {
    const composed = composeTaskDefinition(
      base([{ name: 'LOG_LEVEL', value: 'info' }, { name: 'DD_VERSION', value: 'release-old' }]),
      { projectName: 'hoax', image, secrets, version: 'release-new' }
    )
    expect(composed.containerDefinitions[0].environment).toEqual([
      { name: 'LOG_LEVEL', value: 'info' },
      { name: 'DD_VERSION', value: 'release-new' }
    ])
  })

  it('is a no-op on environment when no version is given', () => {
    const withEnv = composeTaskDefinition(base([{ name: 'LOG_LEVEL', value: 'info' }]), { projectName: 'hoax', image, secrets })
    expect(withEnv.containerDefinitions[0].environment).toEqual([{ name: 'LOG_LEVEL', value: 'info' }])
    // template had no environment array -> none is fabricated
    const withoutEnv = composeTaskDefinition(base(), { projectName: 'hoax', image, secrets })
    expect(withoutEnv.containerDefinitions[0]).not.toHaveProperty('environment')
  })

  it('never touches a sidecar container environment', () => {
    const composed = composeTaskDefinition(base(), { projectName: 'hoax', image, secrets, version: 'release-new' })
    expect(composed.containerDefinitions[1].environment).toEqual([{ name: 'DD_VERSION', value: 'sidecar-untouched' }])
  })

  it('does not mutate the source task definition', () => {
    const source = base([{ name: 'DD_VERSION', value: 'release-old' }])
    composeTaskDefinition(source, { projectName: 'hoax', image, secrets, version: 'release-new' })
    expect(source.containerDefinitions[0].environment).toEqual([{ name: 'DD_VERSION', value: 'release-old' }])
  })
})
